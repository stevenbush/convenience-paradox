...
# Keep forum transcripts readable and responsive in the dashboard.
MAX_FORUM_GROUP_SIZE = 4

# Only the most recent turns are injected back into the next speaker prompt.
MAX_DIALOGUE_CONTEXT_TURNS = 6

# Default number of dialogue turns per group.
DEFAULT_NUM_TURNS = 2

# Shared opening prompt used both by the batch forum runner and the
# incremental Dash UI executor so the two modes stay behaviourally aligned.
FORUM_TOPIC_PROMPT = (
...
def select_forum_groups(
    model: Any,
    forum_fraction: float = DEFAULT_FORUM_FRACTION,
    group_size: int = DEFAULT_GROUP_SIZE,
    rng_seed: int | None = None,
) -> list[list["Resident"]]:
    """Sample and chunk the current model population into forum groups.

    This helper keeps the group-selection logic consistent across the full
    batch forum run and the incremental Dash UI runner. The UI can stage all
    groups immediately, then execute them one turn at a time without drifting
    from the same social sampling rule used by the model-side forum mode.

    Args:
        model: Active ConvenienceParadoxModel instance.
        forum_fraction: Fraction of agents invited into the forum event.
        group_size: Number of agents per discussion group.
        rng_seed: Optional deterministic seed for tests.

    Returns:
        A list of complete agent groups. Incomplete groups are dropped.
    """
    ...
    groups_agents: list[list["Resident"]] = [
        participants[i:i + group_size]
        for i in range(0, len(participants), group_size)
    ]
    return [group for group in groups_agents if len(group) >= 2]


def plan_forum_groups(
    total_agents: int,
    forum_fraction: float = DEFAULT_FORUM_FRACTION,
    group_count: int = 3,
    participant_count: int | None = None,
) -> dict[str, Any]:
    """Derive a lightweight, UI-friendly forum grouping plan.

    The user controls:
    - fraction of the population invited to the forum
    - number of desired groups

    The plan preserves the requested invited fraction as much as possible,
    but reduces the actual number of groups whenever needed to guarantee that
    every group contains at least 2 agents. Group sizes are distributed as
    evenly as possible to keep transcripts comparable.

    Args:
        total_agents: Size of the current simulation population.
        forum_fraction: Fraction of agents invited into the forum event.
        group_count: Desired number of groups.

    Returns:
        A dict describing the feasible participant count, actual group count,
        and the evenly distributed group sizes.
    """
    total_agents = max(0, int(total_agents or 0))
    requested_group_count = max(1, int(group_count or 1))
    requested_participant_count = int(participant_count or 0)

    if total_agents < 2:
        return {
            "participant_count": 0,
            "requested_participant_count": max(0, requested_participant_count),
            "requested_group_count": requested_group_count,
            "actual_group_count": 0,
            "group_sizes": [],
            "fraction_adjusted": False,
            "participant_count_adjusted": requested_participant_count > 0,
            "group_count_adjusted": True,
            "participant_source": "count" if requested_participant_count > 0 else "fraction",
        }

    if requested_participant_count > 0:
        requested_participants = max(2, requested_participant_count)
        participant_source = "count"
    else:
        requested_participants = max(2, int(round(total_agents * float(forum_fraction or 0.0))))
        participant_source = "fraction"

    participant_count = min(total_agents, requested_participants)
    participant_count_adjusted = participant_count != requested_participants

    max_complete_groups = max(1, participant_count // 2)
    actual_group_count = min(requested_group_count, max_complete_groups)

    max_readable_participants = actual_group_count * MAX_FORUM_GROUP_SIZE
    if participant_count > max_readable_participants:
        participant_count = max_readable_participants
        participant_count_adjusted = True

    base_size, remainder = divmod(participant_count, actual_group_count)
    group_sizes = [
    ...
    return {
        ...
        "requested_participant_count": requested_participants,
        "requested_group_count": requested_group_count,
        "actual_group_count": actual_group_count,
        "group_sizes": group_sizes,
        "fraction_adjusted": requested_participants != participant_count,
        "participant_count_adjusted": participant_count_adjusted,
        "group_count_adjusted": actual_group_count != requested_group_count,
        "participant_source": participant_source,
    }


def select_forum_groups_by_count(
    model: Any,
    forum_fraction: float = DEFAULT_FORUM_FRACTION,
    group_count: int = 3,
    participant_count: int | None = None,
    ...
    """Sample and distribute residents into a requested number of forum groups."""
    all_agents = list(model.agents)
    ...
    if participant_count < 2 or not group_sizes:
        return []

    rng = random.Random(rng_seed) if rng_seed is not None else random
    participants = rng.sample(all_agents, participant_count)

    groups: list[list["Resident"]] = []
    cursor = 0
    for size in group_sizes:
        groups.append(participants[cursor:cursor + size])
        cursor += size
    return groups
...
def run_forum_turn(
    ...
    group_agents: list["Resident"],
    prior_turns: list[DialogueTurn] | None,
    *,
    step: int,
    recorder: Optional["LlmAuditRecorder"] = None,
    llm_model: Optional[str] = None,
) -> DialogueTurn:
    """Generate one incremental dialogue turn for a single forum participant.

    Args:
        current_agent: The resident speaking on this turn.
        group_agents: Full participating group so persona context stays grounded.
        prior_turns: Previously generated turns in this group's transcript.
        step: Current simulation step for audit metadata.
        recorder: Optional LLM audit recorder.
        llm_model: Optional Ollama model override.

    Returns:
        One DialogueTurn object containing the generated utterance.
    """
    personas = {agent.unique_id: _build_agent_persona(agent) for agent in group_agents}
    messages = [{"role": "system", "content": FORUM_SYSTEM_PROMPT}]
    messages.append({"role": "user", "content": FORUM_TOPIC_PROMPT})

    for previous_turn in (prior_turns or [])[-MAX_DIALOGUE_CONTEXT_TURNS:]:
        messages.append({"role": "assistant", "content": previous_turn.content})

    speaker_prompt = (
        f"You are: {personas[current_agent.unique_id]}\n\nPlease respond to the ongoing discussion."
        if not prior_turns else
    ...
    try:
        ...
        if recorder:
            recorder.record_call(
                ...
                extra={
                    "step": step,
                    "speaker_id": current_agent.unique_id,
                    "group_agent_ids": [agent.unique_id for agent in group_agents],
                },
            )
    ...
    return DialogueTurn(
        speaker_id=current_agent.unique_id,
        speaker_label=personas[current_agent.unique_id].split(":")[1].strip()[:60],
        content=content,
    )


def extract_forum_outcome_from_turns(
    ...
    *,
    ...
    """Extract one group's norm signal from an existing turn list."""
    transcript = "\n".join(
        f"Resident #{turn.speaker_id % 100}: {turn.content}"
        for turn in (turns or [])
    )
    return _extract_forum_outcome(
        transcript,
        recorder=recorder,
        step=step,
        agent_ids=agent_ids,
...
def apply_forum_outcome(
    group_agents: list["Resident"],
    outcome: Optional[ForumOutcome],
) -> tuple[float, list[dict[str, Any]]]:
    """Apply one bounded forum norm update to a group of agents.

    Returns:
        Tuple of ``(delta_applied, preference_updates)`` where
        ``preference_updates`` mirrors the audit payload shown in the UI.
    """
    if outcome is None:
        return 0.0, []
    ...
    return delta, preference_updates
...
def run_forum_step(
    ...
    """
    t0 = time.perf_counter()
    groups_agents = select_forum_groups(
        model,
        forum_fraction=forum_fraction,
        group_size=group_size,
        rng_seed=rng_seed,
    )
    n_invite = sum(len(group) for group in groups_agents)
    ...
    for group in groups_agents:
        ...
        if group_session.outcome:
            ...
            delta, preference_updates = apply_forum_outcome(group, outcome)
            group_session.delta_applied = delta
            group_session.preference_updates = preference_updates
